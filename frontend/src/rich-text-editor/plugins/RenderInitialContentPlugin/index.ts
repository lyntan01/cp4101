...
export type TextSerializedLexicalNode = {
  text: string;
  detail: number;
  style: string;
} & SerializedLexicalNode;

const isTextSerializedNode = (
  node: SerializedLexicalNode
): node is TextSerializedLexicalNode => {
...
const recursiveShortenText = (nodes: SerializedLexicalNodeWithChildren[]) => {
  return nodes.map((node) => {
    ...
    if (node.children) {
      node.children = recursiveShortenText(node.children);
    }
    return node;
  });
};
  ...
  useLayoutEffectImpl(() => {
    ...
    timerId = setTimeout(() => {
      ...
      if (shorten) {
        ...
        shortened.root.children = recursiveShortenText(shortened.root.children);
