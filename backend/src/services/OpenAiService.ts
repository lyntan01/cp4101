...
export class OpenAiService {
  constructor(
    ...
    private generatedExerciseService: GeneratedExerciseService = new GeneratedExerciseService()
  ) { }
...
"isBinary": false
    ...
    const sandboxId = await this.codeSandboxService.createSandbox(
      parsedJson.exercise.files
    )

    // Save exercise page instructions as markdown
    // Markdown will be converted to Lexical JSON when first retrieved on the frontend
    const newExercisePage = await this.exercisePageService.createExercisePage({
      title: parsedJson.title,
      chapterId: chapterId,
  ...
  public async generateNewExercise({
    chapterId,
    chapterName,
    chapterLearningOutcomes,
    studentPreviousExerciseInstructions,
    studentPreviousExerciseAttempt, // Marked as optional
    studentPreviousExerciseFeedback // Marked as optional
  }: {
    chapterId: string
    chapterName: string
    chapterLearningOutcomes: string[]
      ...
      messages: [
        {
          role: 'system',
          content: `You are a helpful assistant designed to output JSON.
        Provide your answer in JSON structure like this {"instructions": "<Typescript string containing exercise instructions in Markdown>", "exercise": "<File contents in JSON format>", "correctAnswer": "<String containing correct answer>"}`
        },
        {
          role: 'user',
          name: 'Instructor',
...
"isBinary": false
        ...
        },
        {
          role: 'user',
          name: 'Instructor',
          content: `${getGenerateNewExercisePrompt(
            chapterName,
            chapterLearningOutcomes,
            studentPreviousExerciseInstructions,
            studentPreviousExerciseAttempt,
            studentPreviousExerciseFeedback
          )}`
        }
      ]
    })
    ...
    // Save exercise instructions as markdown
    // Markdown will be converted to Lexical JSON when first retrieved on the frontend
    const newGeneratedExercise = await this.generatedExerciseService.createGeneratedExercise({
      chapterId: chapterId,
      instructions: parsedJson.instructions,
      sandboxId: sandboxId,
      correctAnswer: parsedJson.correctAnswer
...
function getExercisePagePrompt(
  ...
  prompt += `The exercise should consist of three parts. 
...
function getGenerateNewExercisePrompt(
  chapterName: string,
  learningOutcomes: string[],
  studentPreviousExerciseInstructions: string,
  studentPreviousExerciseAttempt?: string,
  ...
  prompt += `The exercise should consist of three parts. 
    ...
    Here is the previous exercise for reference:
  ...
  if (studentPreviousExerciseAttempt) {
    prompt += `Here is the previous exercise attempt and feedback for reference:
    Previous exercise attempt by student: ${studentPreviousExerciseAttempt}`

    if (studentPreviousExerciseFeedback) {
      prompt += `Previous exercise AI-generated feedback given to student: ${studentPreviousExerciseFeedback}`
    }

    prompt += `You MUST generate an exercise that target's the student's weaknesses as demonstrated in the previous exercise, 
    is challenging and builds upon the previous exercise.`
  }
...
function getExplorationPagePrompt(
  ...
  prompt += `The exploration should consist of two parts.
